...
jgcricol <- function(palx=NULL, alpha=NULL) {
  ...
  # General purpose color scheme where sequentail jgcricol do not clash too much
  ...
  # Basic jgcricol
  pal_basic <- c("firebrick3","dodgerblue3","forestgreen","black","darkgoldenrod3","darkorchid3","gray50", "darkturquoise")
  ...
  pal_all_lower <- pal_all;
  names(pal_all_lower) <- tolower(names(pal_all_lower))
  pal_all_upper <- pal_all;
  names(pal_all_upper) <- toupper(names(pal_all_upper))
  pal_all_title <- pal_all;
  names(pal_all_title) <- stringr::str_to_title(names(pal_all_title))

  # Combined Palette
  pal_all <- c(pal_all,pal_all_lower,pal_all_upper,pal_all_title)
  pal_all <- pal_all[(names(pal_all)[!duplicated(names(pal_all))])]

  # Checks
  # names(pal_all)[names(pal_all) %>% duplicated()]


  if(length(names(pal_all)[duplicated(names(pal_all))])>0){
    stop(paste("Duplicated color names in pal_all: ",paste(names(pal_all)[duplicated(names(pal_all))],collapse=", "),sep=""))
  }
  ...
  if(!is.null(palx)){
    if(length(get(palx))>1){
      mypal<-get(palx)
      if(is.null(names(mypal))){names(mypal)=mypal}
      if(!is.null(alpha)){
